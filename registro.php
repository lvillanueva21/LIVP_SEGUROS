    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-9 text-center mb-5">
                <h2 class="heading-section">Registro temporal de usuario Desarrollo</h2>
            </div>
        </div>
                            ...
                            <div class="carousel-inner">
                                <?php foreach ($assets['carrusel'] as $index => $image): ?>
                                    <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                                        <img src="<?= e($image['url']) ?>" class="d-block w-100 js-cover-image" alt="Portada de registro <?= $index + 1 ?>" draggable="false">
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    ...
                    <div class="login-wrap p-4 p-md-5">
                        <div class="d-flex align-items-center mb-2">
                            ...
                            <div class="w-100">
                                <p class="social-media d-flex justify-content-end m-0">
                                    <a href="<?= e(appRelativeUrl('index.php')) ?>" class="social-icon d-flex align-items-center justify-content-center" title="Ir al login" aria-label="Ir al login"><span class="fa fa-sign-in"></span></a>
                                </p>
                            </div>
                        </div>

                        <?php if ($notice !== ''): ?>
                            <div class="alert alert-success py-2 mb-3" role="alert"><?= e($notice) ?><br><a href="<?= e(appRelativeUrl('index.php')) ?>" class="alert-link">Ir al login</a></div>
                        <?php endif; ?>
                        <?php if ($error !== ''): ?>
                        ...
                        <form method="post" class="signin-form" autocomplete="off" novalidate>
                            ...
                            <div class="form-group mt-3">
                                <label for="nombres" class="form-label-fixed">Nombres</label>
                                <input id="nombres" type="text" name="nombres" class="form-control" maxlength="120" required value="<?= e($form['nombres']) ?>">
                            </div>
                            <div class="form-group">
                                <label for="apellidos" class="form-label-fixed">Apellidos</label>
                                <input id="apellidos" type="text" name="apellidos" class="form-control" maxlength="120" required value="<?= e($form['apellidos']) ?>">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="document" class="form-label-fixed">Número de DNI</label>
                                <input id="document" type="text" name="document" class="form-control" inputmode="numeric" pattern="\d{8}" maxlength="8" required value="<?= e($form['document']) ?>">
                            </div>
                            <div class="form-group">
                                <label for="password-field" class="form-label-fixed">Contraseña</label>
                                <div class="position-relative">
                                    <input id="password-field" type="password" name="password" class="form-control" minlength="8" autocomplete="new-password" required>
                                    <span toggle="#password-field" class="fa fa-fw fa-eye field-icon toggle-password" title="Mostrar u ocultar"></span>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="password-repeat-field" class="form-label-fixed">Repetir contraseña</label>
                                <div class="position-relative">
                                    <input id="password-repeat-field" type="password" name="password_repeat" class="form-control" minlength="8" autocomplete="new-password" required>
                                    <span toggle="#password-repeat-field" class="fa fa-fw fa-eye field-icon toggle-password" title="Mostrar u ocultar"></span>
                                </div>
                            </div>
                            <div class="form-group">
                                <button class="form-control btn btn-primary rounded submit px-3" type="submit">Crear usuario Desarrollo</button>
                            </div>
                        </form>
