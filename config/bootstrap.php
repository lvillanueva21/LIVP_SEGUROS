...
function appRelativeUrl(string $target = ''): string
{
    ...
    return str_repeat('../', $currentDirectory === '' ? 0 : count(explode('/', $currentDirectory))) . ltrim($target, '/');
}
...
function sessionCookiePathForProject(): string
{
    ...
    if ($relativeScript !== '' && str_ends_with($scriptName, $relativeScript)) {
        $basePath = '/' . trim((string) substr($scriptName, 0, -strlen($relativeScript)), '/');
...
require_once __DIR__ . '/login_assets.php';
...
function isAuthenticated(): bool
{
    ...
    foreach (['id', 'role', 'role_label', 'name', 'document_type', 'document'] as $key) {
        if (!array_key_exists($key, $_SESSION['livp_user'])) {
            return false;
        }
    }
...
function createUserSession(array $user): void
{
    ...
    $labels = ['persona' => 'Persona', 'empresa' => 'Empresa', 'consorcio' => 'Consorcio'];
    $type = (string) ($user['account_type'] ?? 'persona');
    $user['account_type'] = array_key_exists($type, $labels) ? $type : 'persona';
    $user['account_type_label'] = $labels[$user['account_type']];
...
function requireAuth(): array
{
    ...
    if (!authCurrentDatabaseSessionIsActive()) {
        destroyCurrentSession();
        sendNoCacheHeaders();
        header('Location: ' . appRelativeUrl('index.php?m=acceso_actualizado'));
        exit;
    }
...
function modulesCatalog(): array
{
    ...
    if ($modules === null) {
        $loaded = require __DIR__ . '/modules.php';
        $modules = is_array($loaded) ? $loaded : [];
    }
...
function moduleUrl(string $moduleId): string
{
    ...
    return isset($custom[$moduleId])
        ? appRelativeUrl($custom[$moduleId])
        : appRelativeUrl('modulo.php?modulo=' . rawurlencode($moduleId));
}
